...
export class CommentController {
  ...
  async websiteGetEcommerceProductComment(
    ...
    try {
      this.logger.log(`${this.websiteGetEcommerceProductComment.name} called`);
      const result = await this.commentService.getEcommerceProductComment(productId);
      return res.status(200).json(result);
    } catch (error) {
      this.logger.log(`${this.websiteGetEcommerceProductComment.name} Error:${error.message}`);
      throw new HttpException(error.message, error.status || 500);
    } finally {
      this.logger.log(`${this.websiteGetEcommerceProductComment.name} Done`);
    }
  }
}
